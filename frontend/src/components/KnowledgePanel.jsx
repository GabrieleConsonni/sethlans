...
// Pannello consultabile della conoscenza di progetto:
// - il profilo del progetto (specchio di CLAUDE.md + config, gestito da /sethlans-onboard);
// - le knowledge card (per ruolo/ambito) prodotte dal pre-training.
// Read-only: gli agenti scrivono via API, qui si consulta soltanto.
...
const KIND_LABELS = { profile: "Profilo", kb: "KB", learnings: "Appresi" };
...
export default function KnowledgePanel({ state, selectedProject }) {
  ...
  if (!project) {
    return <div className="empty">Nessun progetto selezionato.</div>;
  }

  // ordina: prima il profilo, poi per ruolo
    ...
    <div style={{ maxWidth: 920, margin: "0 auto", padding: "8px 4px 40px" }}>
      ...
      <section style={{ marginBottom: 22 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
          <h3 style={{ margin: 0, fontSize: 15 }}>Profilo progetto</h3>
        ...
        {project.md && project.md.trim() ? (
          ...
          <div className="empty">
            Nessun profilo. Esegui <code>/sethlans-onboard</code> per popolarlo da CLAUDE.md.
          </div>
        )}
        {hasConfig && (
          <details style={{ marginTop: 12 }}>
            <summary style={{ cursor: "pointer", fontSize: 13, color: "var(--muted)" }}>
              Config per-ruolo (puntatori)
            </summary>
            <pre
              style={{
                fontSize: 12,
      ...
      <section>
        <h3 style={{ margin: "0 0 10px", fontSize: 15 }}>
          Knowledge card <span style={{ color: "var(--muted)", fontWeight: 400 }}>({sorted.length})</span>
        </h3>
        {sorted.length === 0 ? (
          <div className="empty">
            Nessuna card. Il pre-training (<code>/sethlans-onboard</code>) le crea per PO, architect, UX, tester.
          </div>
        ) : (
