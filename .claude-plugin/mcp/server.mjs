#!/usr/bin/env node
// Tabula MCP server — wrapper sottile sopra l'API REST di Tabula.
// =============================================================================
...
//
// Principio guida (come il resto del protocollo Tabula): aggiornare la board è
// best-effort e MAI bloccante. Se il board non risponde, il tool ritorna un
// errore "soft" (isError) con un messaggio chiaro: l'agente prosegue il lavoro.
//
// Config: TABULA_API_URL (default http://localhost:9955).
// =============================================================================

const BASE = (process.env.TABULA_API_URL || "http://localhost:9955")
  .trim()
  .replace(/\/+$/, "") || "http://localhost:9955";

const SERVER_INFO = { name: "tabula", version: "1.0.0" };
...
const TOOLS = [
...

  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        agent_name: { type: "string", description: "Nome canonico dell'agente (es. 'be-python'); registrato se assente." },
  ...
  {
    name: "tabula_get_or_register_agent",
    description:
    ...
    inputSchema: {
      type: "object",
      properties: {
        name: { type: "string", description: "Nome canonico (es. 'frontend', 'architect')." },
        ...
        tokens: { type: "integer", description: "Valore assoluto dei token (per incrementi usa tabula_add_agent_tokens)." },
      },
      required: ["name"],
    },
...

  {
    name: "tabula_request",
    description:
      "Escape hatch di basso livello: chiamata REST arbitraria all'API Tabula. Da usare " +
...
async function handleToolCall(name, args) {
  ...
  try {
    ...
    const hint = err instanceof BoardError && err.unreachable
      ...
      : "";
    return {
      content: [{ type: "text", text: `Errore Tabula: ${err.message}${hint}` }],
      isError: true,
    };
  }
}
